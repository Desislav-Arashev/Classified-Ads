...
function ShowFields(id)
{
	...
	/*
	var s_div = "packages-"+id;
	
	var elms = document.getElementById("ad-packages").getElementsByTagName("div");
	var all_show = true;
	if(document.getElementById(s_div))
	{
		document.getElementById(s_div).style.display="block";
    ...
    for (var i = 0; i < elms.length; i++) 
	{
		if(elms[i].id.indexOf("packages") == 0)
		{
			if(elms[i].id!=s_div && document.getElementById(elms[i].id))
			{
				if(all_show)
				{
	...
	*/
	...
	/*
	if(main_cat_ids.contains(id))
	{
		for (var i = 0; i < main_cat_ids.length; i++) 
		{
			if(main_cat_ids[i]==id)
			{
				if(document.getElementById("fields-"+main_cat_ids[i]))
				{
					document.getElementById("fields-"+main_cat_ids[i]).style.display="block";
				}
			}
			else
			{
				if(document.getElementById("fields-"+main_cat_ids[i]))
				{
					document.getElementById("fields-"+main_cat_ids[i]).style.display="none";	
				}
			}
		}
	}
